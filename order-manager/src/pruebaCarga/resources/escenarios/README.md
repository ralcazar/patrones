...
| `reclamo_perdido` | `motivo` = `TOKEN_VIGENTE` \| `NO_VIVA` \| `NO_EJECUTABLE` \| `COLISION_OPTIMISTA` | `ServicioContinuarOrden.reclamarToken` / `reclamarYEjecutar` | El pod no consigue el token: la orden ya está finalizada, otro pod tiene un token vigente, la orden ya no está en turno de ejecución (otro actor la aparcó/reprogramó entre el barrido de candidatas y este reclamo, `OrdenRoot.turnoVencido`), o el guardado del reclamo choca por versión |
