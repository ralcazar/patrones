 ...
 * saga + tareas encoladas); fuera de ella solo I/O externo (REST).
 * Así:
...
public abstract class ServicioSagaBase<P extends Enum<P> & PasoSaga, S extends Saga<P>>
    ...
    /** El planificador de tickets ya avisó a soporte: la saga pasa a ABIERTO con su fecha. */
    public void marcarTicketAbierto(SagaId id, Instant apertura) {
        procesar(MensajeId.interno(), id, saga -> saga.marcarTicketAbierto(apertura));
    }
    ...
    /** Parte externa de las decisiones: REST. Nunca dentro de la tx. */
